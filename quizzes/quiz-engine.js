...
(function () {
  ...
  const STORAGE_KEY = "overhang-quiz-progress";
  ...
  function isDevMode() {
    try {
      ...
      if (param === "true")  { localStorage.setItem("overhang-dev-mode", "true");  return true; }
      if (param === "false") { localStorage.removeItem("overhang-dev-mode");        return false; }
      return localStorage.getItem("overhang-dev-mode") === "true";
    } catch { return false; }
  }
  ...
  const _BACKUP_KEY = "overhang-course-v1-progress-integrity-2026";
  const _BACKUP_VER = 1;
  ...
  /**
   * exportProgress()
   * Downloads a signed base64 progress backup as overhang-progress.txt.
  ...
  window.exportProgress = async function () {
    ...
    a.href     = url;
    a.download = "overhang-progress.json";
